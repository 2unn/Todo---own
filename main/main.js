...
import {getListApiVersion, createNewItemApi, updateItemApi, deleteItemApi} from './fetchFunction.js';
...
function render (list,pageIndex = 0,pageSize = 4) {
    ...
    const htmls = listfilter.map((val, index) => {
       
        return ` <tr class = "list">
        <th scope="row">${(pageIndex * pageSize) +index}</th>
        <td class = 'value'>${val.name}</td>
        <td scope="row">
            <button type="button" id = 'edit' class="btn btn-info" onclick="Edit('${val.id}')">Edit</button>
            <button type="button" class="btn btn-warning btn-delete" onclick="Delete('${val.id}')">Delete</button>
        </td>
    </tr>
        ` 
    }).join('')
...
function paginateList (list,pageIndex, pageSize = 4) {
    ...
    const paginationList = document.getElementsByClassName('pagination justify-content-center')[0]
    const countAray = [...Array(Math.ceil(list.length / pageSize))].map((_,index) => +index);


    if(countAray.length > 1){
        pagination.style.display = 'block'
        const paginationListEL = countAray.map((_, index)=>{
           return `<li id="${index}" class="page-item "><a class="page-link" >${index + 1}</a></li>`
        }).join('');
      ...
      `
        ...
        const btnPrevious = document.getElementsByClassName('page-item previous')[0]
...
function listAction (list) {
    ...
    const searchBtn = document.getElementsByClassName('btn btn-warning btn-search')[0];
    ...
    const sortAToZBtn = document.getElementsByClassName('btn btn-warning btn-sort')[0];
        ...
        if (value !== null) {
            if (value !== '' ) {
                listAction.forEach(item => {
                    ...
                    updateItemApi(()=> {
                        getListApiVersion((list) => filterRender(listAction,currentPage))
                    },idChange,value)
                })
            } else {
